...
void ArbolAVL::insertar(FranjaHoraria FranjaHoraria) {
    id = FranjaHoraria.regresarFechanumerica();
...
bool NodoFecha::esDisponible(const FranjaHoraria& nuevaFH) {
    for (Evento* evento : eventos) {
        if (evento->getFranjaHoraria().igualA(nuevaFH) || evento->getFranjaHoraria().antesDe(nuevaFH)) {
            return false; 
        }
    }
    return true;
